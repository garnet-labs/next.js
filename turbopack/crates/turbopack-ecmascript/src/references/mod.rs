...
pub async fn compute_ecmascript_module_exports(
    ...
    let ParseResult::Ok {
        ...
        return Ok(EcmascriptExports::Unknown.cell());
    };

    // Build import references. We use ImportUsage::TopLevel (the default) for all
    // references since we don't compute import usage from the VarGraph. This is
    // conservative but always correct.
    let import_references = {
        let mut refs = Vec::with_capacity(eval_context.imports.references().len());
        for r in eval_context.imports.references() {
            let reference = EsmAssetReference::new(
                ...
                r.annotations.as_ref().map(|a| (**a).clone()),
                match &r.imported_symbol {
                    ImportedSymbol::ModuleEvaluation => Some(ModulePart::evaluation()),
                    ImportedSymbol::Symbol(name) => Some(ModulePart::export((&**name).into())),
                    ImportedSymbol::PartEvaluation(part_id) | ImportedSymbol::Part(part_id) => {
                        if !matches!(
                            options.tree_shaking_mode,
