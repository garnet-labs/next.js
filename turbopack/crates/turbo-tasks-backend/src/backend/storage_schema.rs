...
impl TaskStorage {
    ...
    pub fn evictability(&self) -> (KeyEvictability, DataEvictability) {
        ...
        let data_evictable = flags.data_restored()
            ...
            // If we are storing any cells which contain non-trivial session state then we cannot be evicted
            && self.persistent_cell_data().is_none_or(|m| !m.iter().any(|(k,_)|  k.type_id.is_session_stateful()));
